...
v0.10.3. It is not an OWASP certification, an assertion of a complete ASVS
...
target and expected counts are pinned in `audit/amendments/v0.10.3.toml`.
...
| HTTPS auth cookies are Secure, use and exclusively accept the `__Host-` name; plain HTTP accepts only the bare local-development name | `v5.0.0-3.3.1`, `v5.0.0-3.3.3` | covered | `tests/test_cookie_boundaries.py`, `tests/test_email_flow.py::test_https_uses_host_prefix_and_secure`, `::test_session_cookie_acceptance_is_bound_to_the_request_scheme`, `tests/test_oauth.py::test_https_callback_rejects_plain_http_state_cookie` |
