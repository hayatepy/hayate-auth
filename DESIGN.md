...
| **API キー**(create/verify/list/delete、scope/expiry、ハッシュ保存)v0.5 | rate limit / refill の内蔵カウンタ | キー本体は採るが、スロットリングは §9 どおりインフラ責務 |
| **TOTP 2FA**(RFC 6238)v0.4 | organization / SSO / passkey の先行整備 | 標準準拠で価値が高いものから。証拠駆動 |

> better-auth 参考の採用は 2026-07-23 に再確認(better-auth の現行プラグイン: API Key /
> OAuth Provider(AS)/ JWT / OpenAPI / Agent Auth 等)。**API キーと TOTP を house style
> (純関数・ゼロ依存・標準ファースト)で移植**。OAuth Provider(AS モード)は mcp の RS と
> 合流する本命として v0.6+。JWT 署名(RS256/ES256)は stdlib 制約のため extra 判断(§17-2)。
> OpenAPI は hayate-openapi のレシピで対応(重複実装しない)。
...
- `auth.verify_api_key(key)` — API キー検証 → `{user_id, scopes, key_id, name} | None`。
  **hayate-mcp の `Authorization(verify_token=auth.verify_api_key)` に直結**(API キーで MCP 保護)
...
| v0.6 | passkey(`[passkey]` extra)/ magic link / プラグイン API 抽出 / **AS モード**(mcp と合流) | — |
