...
v0.9.1. The goal is to identify exploitable defects and unsafe integration
assumptions before recommending the library as the sole authentication layer
for production credentials. This is a code and design audit, not an OWASP
...
Review the frozen target and boundaries in `target.toml` and
`THREAT_MODEL.md`, including the core, SQLite/D1 adapters, OAuth client and
authorization-server modes, sessions, recovery, TOTP, magic link, passkeys,
API keys, MCP integration, release provenance, and integration guidance.

Deliver:

1. A kickoff scope confirmation and list of exclusions.
2. A threat-model/design review and manual source review.
3. Reproduction of Profiles A–C in `PROCEDURES.md`, with environment records.
4. Exploit or failing-test evidence for each confirmed finding when safe.
5. A report with severity, affected target, preconditions, impact,
   reproduction, remediation, and references.
...
not infer coverage solely from existing tests.
